...
RESULTS_DIR = "scripts-studies/how-to-expand/results-random-ofa"
...
def model_evaluator(
    ...
    identifiers = [
        "ofa" if weights_from_ofa else "random",
        f"seed{random_seed}",
    ]
...
if __name__ == "__main__":
    ...
    for i in tqdm(range(N_MODELS), desc="Evaluation", unit="model"):
        ...
        # Evaluate random initialisation, no distillation
        print("\n" + "-" * 90)
        print("Evaluating random initialisation")
        base_evaluator(weights_from_ofa=False)

        # Evaluate ofa initialisation, no distillation
        print("\n" + "-" * 90)
        print("Evaluating ofa initialisation")
        base_evaluator(weights_from_ofa=True)
