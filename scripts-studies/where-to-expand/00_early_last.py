...
RESULTS_DIR = "scripts-studies/where-to-expand/results-early-last"
...
def model_evaluator(
    ...
    identifiers = [
        f"{'last' if expand_from_end else 'early'}",
        f"seed@{random_seed}",
    ]
    ...
    trainer = GrowingDataContinualTrainer(
        capacity_tau=CAPICITY_TAUS[dataset_name],
    ...
    # Set the search space expansion direction
    trainer.search_space.expand_from_end = expand_from_end
...
if __name__ == "__main__":
    ...
    for i in tqdm(range(N_MODELS), desc="Evaluation", unit="model"):
        ...
        # Evaluate expansion from early layers
        print("\n" + "-" * 90)
        print("Evaluating start from early")
        base_evaluator(expand_from_end=False)

        # Evaluate expansion from last layers
        print("\n" + "-" * 90)
        print("Evaluating start from last")
        base_evaluator(expand_from_end=True)
