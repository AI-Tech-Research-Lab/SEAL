...
RESULTS_DIR = "scripts-studies/when-to-expand/results-capacities"
...
# Define the capacities we are testing on the models
CAPACITIES_TO_CHECK = [0.045, 0.08, 0.13, 0.2, 0.3]
...
if __name__ == "__main__":
    ...
    for i in tqdm(range(N_MODELS), desc="Evaluation", unit="model"):
        ...
        for iter_capacity_tau in CAPACITIES_TO_CHECK:
            print("\n" + "-" * 90)
            print(f"Evaluating tau@{iter_capacity_tau}")

            model_evaluator(
                capacity_tau=iter_capacity_tau,
                model=iter_model,
