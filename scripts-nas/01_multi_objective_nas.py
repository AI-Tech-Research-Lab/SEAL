...
from nas.continual_nas import EfficientContinualMultiNAS
import argparse
import shutil
import yaml
import os
...
if __name__ == "__main__":
    ...
    multi_objective_dir = os.path.join(experiment_dir, "multi-objective")
    os.makedirs(multi_objective_dir, exist_ok=True)
    shutil.copy(config_filepath, os.path.join(multi_objective_dir, "config.yaml"))

    # Parse parameters and run the main loop
    nas = EfficientContinualMultiNAS(
        ...
        first_obj_surrogate_name=config["surrogate_models"]["first-obj"],
        second_obj_surrogate_name=config["surrogate_models"]["second-obj"],
